      ...
      <div className="max-h-[92vh] w-full max-w-[680px] overflow-y-auto rounded-2xl bg-[#fffdf8] p-[24px] shadow-2xl md:p-[48px]" role="dialog" aria-modal="true" aria-labelledby="interest-title">
        ...
        {status === "success" ? <div className="rounded-xl bg-[#eaf8f5] p-[24px] font-['Nunito:Regular',sans-serif] text-[length:var(--ara-text-body)] leading-[1.5] text-[#006057]" role="status">{message}</div> : (
          <form className="grid gap-[24px]" onSubmit={handleSubmit}>
            <label className="grid gap-[12px] font-['Nunito:SemiBold',sans-serif] text-[length:var(--ara-text-body)] font-semibold leading-[1.5] text-[#2d251d]">Email address<input required name="email" type="email" autoComplete="email" className="rounded-lg border-2 border-[#dfdac9] bg-white px-[24px] py-[16px] font-['Nunito:Regular',sans-serif] font-normal leading-[1.5] transition-all focus:border-[#00a193] focus:outline-none" /></label>
            <label className="grid gap-[12px] font-['Nunito:SemiBold',sans-serif] text-[length:var(--ara-text-body)] font-semibold leading-[1.5] text-[#2d251d]">Country of residence<select required name="residenceCountry" defaultValue="" className="rounded-lg border-2 border-[#dfdac9] bg-white px-[24px] py-[16px] font-['Nunito:Regular',sans-serif] font-normal leading-[1.5] transition-all focus:border-[#00a193] focus:outline-none"><option value="" disabled>Select a country</option>{countries.map(c => <option key={c.code} value={c.code}>{c.name}</option>)}</select></label>
            <label className="grid gap-[12px] font-['Nunito:SemiBold',sans-serif] text-[length:var(--ara-text-body)] font-semibold leading-[1.5] text-[#2d251d]">Language you want for your child<select required name="desiredLanguage" value={language} onChange={e => setLanguage(e.target.value)} className="rounded-lg border-2 border-[#dfdac9] bg-white px-[24px] py-[16px] font-['Nunito:Regular',sans-serif] font-normal leading-[1.5] transition-all focus:border-[#00a193] focus:outline-none">{desiredLanguages.map(item => <option key={item}>{item}</option>)}</select></label>
            {language === "Other" && <label className="grid gap-[12px] font-['Nunito:SemiBold',sans-serif] text-[length:var(--ara-text-body)] font-semibold leading-[1.5] text-[#2d251d]">Which language?<input required name="otherLanguage" className="rounded-lg border-2 border-[#dfdac9] bg-white px-[24px] py-[16px] font-['Nunito:Regular',sans-serif] font-normal leading-[1.5] transition-all focus:border-[#00a193] focus:outline-none" /></label>}
            <label className="grid gap-[12px] font-['Nunito:SemiBold',sans-serif] text-[length:var(--ara-text-body)] font-semibold leading-[1.5] text-[#2d251d]">Child age range<select required name="childAgeRange" defaultValue="" className="rounded-lg border-2 border-[#dfdac9] bg-white px-[24px] py-[16px] font-['Nunito:Regular',sans-serif] font-normal leading-[1.5] transition-all focus:border-[#00a193] focus:outline-none"><option value="" disabled>Select an age range</option>{childAgeRanges.map(item => <option key={item}>{item}</option>)}</select></label>
            ...
            <button disabled={status === "submitting"} className="w-full rounded-lg bg-[#00a193] px-[43px] py-[16px] font-['Inter:Semi_Bold',sans-serif] text-[length:var(--ara-text-lead)] font-semibold leading-[1.2] text-white transition-all hover:bg-[#008d80] disabled:cursor-not-allowed disabled:bg-gray-400 disabled:opacity-60">{status === "submitting" ? "Saving…" : featuredProductStage.cta}</button>
